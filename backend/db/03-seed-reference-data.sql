...
/*
  Development-only user accounts.
  Replace PasswordHash with bcrypt hashes before production login is enabled.
*/
DECLARE @ItOps3 UNIQUEIDENTIFIER = (SELECT DepartmentId FROM app.Departments WHERE Code = N'ITOPS');
...
USING (VALUES
  (N'requester@test.com', N'DEV_ONLY_password123', N'Mia Santos', @ItOps3),
  (N'endorser@test.com', N'DEV_ONLY_password123', N'Noel Tan', @ItOps3),
...
WHEN MATCHED THEN UPDATE SET FullName = source.FullName, DepartmentId = source.DepartmentId, IsActive = 1, UpdatedAt = SYSUTCDATETIME()
WHEN NOT MATCHED THEN INSERT (Email, PasswordHash, FullName, DepartmentId) VALUES (source.Email, source.PasswordHash, source.FullName, source.DepartmentId);
GO

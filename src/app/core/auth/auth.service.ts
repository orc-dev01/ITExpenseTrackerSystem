...
export class AuthService {
  ...
  readonly isAuthenticated = computed(() =>
    Boolean(this.sessionState()?.accessToken),
  );

  login(command: LoginCommand) {
    // console.log("mockAuth =", environment.mockAuth);
    if (environment.mockAuth) {
      ...
      if (!account) {
        ...
        return throwError(() => ({
          status: 401,
          error: {
            message: "Invalid mock account credentials.",
          },
        }));
      }
    ...
    return this.api
      .post<AuthSession>(ApiEndpoints.auth.login, command)
